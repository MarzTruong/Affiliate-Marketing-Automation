...
- **[2026-04-26] Repo restructure (phiên 15):** Xóa shim folders `backend/connectors/` và `backend/publisher/` (chỉ có `__init__.py` forward đến `backend.affiliate.*`). Xóa connector lỗi `backend/affiliate/connectors/tiktok_shop.py` (HMAC signing bug). Docs được tổ chức lại: `docs/operations/` (SOP files), `docs/content/` (content strategy), `docs/setup/` (platform setup guides). TikTok verification `.txt` files → `backend/static/root/` + mounted trong `main.py`. HTML files → `frontend/public/`. Production code split: `kenh1_production.py` (Kling+Gemini), `kenh2_production.py` (HeyGen+ElevenLabs), `production.py` là thin dispatcher.

- **[2026-04-26] HMAC signing bug trong old TikTokShopConnector:** `backend/affiliate/connectors/tiktok_shop.py` sai spec: `access_token` bị đưa vào HMAC signing string (không được phép). Đúng spec: chỉ sign `app_key + timestamp + sorted_params` (không có `access_token`). Thêm vào đó: `except: return []` và `except: return AffiliateLink(...)` — silent error swallowing. HTTP client leak: không có `async with` hoặc `close()`. Fix: xóa hoàn toàn, dùng `backend/tiktok_shop/connector.py` (đúng spec, typed exceptions).

- **[2026-04-26] TikTok domain verification files trả 404:** Ba file `.txt` TikTok verification (`docs/tiktokBtP04IrENq7z4P4vyU3ZW0vgfxiCxgkL.txt`, v.v.) nằm trong `docs/` nhưng không được serve. Fix: move sang `backend/static/root/` + mount `StaticFiles(directory="backend/static/root")` ở root path trong `main.py`. Hệ thống đang dùng GitHub Pages (`docs/`) nên các file này đã serve đúng cho TikTok URL verification — tuy nhiên backend cần serve chúng ở `/{filename}` cho các lần verify sau.

- **[2026-04-26] 2-channel code architecture mapping:** Kênh 1 "Lab Gia Dụng" (faceless): `kenh1_production.py` (Kling + Gemini TTS), `kenh1_publisher.py` (TikTokPublisher). Kênh 2 "Đồ Này Tui Xài" (semi-auto): `kenh2_production.py` (ElevenLabs + HeyGen), `kenh2_studio.py` (TikTok Studio API). `production.py` = thin dispatcher, nhận `channel_type` từ `TikTokProject.channel_type` field (migration `151409122f54`).
